import Link from "next/link";
import SignupForm from "@/components/features/auth/SignupForm";
import Button from "@/components/ui/Button";

/**
 * ユーザー登録ページ。
 * @returns 登録フォームを含むページ
 */
export default function UserCreatePage() {
  return (
    <main className="min-h-screen pt-24 bg-page-bg">
      <div className="w-full max-w-md mx-auto bg-white rounded-2xl shadow-md px-10 py-12">
        {/* ロゴ・サービス説明 */}
        <h1 className="text-3xl font-bold text-primary text-center tracking-tight">
          Enthink
        </h1>
        <p className="mt-2 text-sm text-text-secondary text-center">
          思考の言語化を通じて、数学的思考力を鍛えるEdTechサービス
        </p>

        <SignupForm />

        {/* 区切り */}
        <div className="flex items-center gap-3 my-5">
          <div className="flex-1 h-px bg-border" />
          <span className="text-sm text-text-tertiary">または</span>
          <div className="flex-1 h-px bg-border" />
        </div>

        {/* Google OAuth ボタン（未実装） */}
        <Button
          type="button"
          className="w-full flex items-center justify-center gap-2 py-3 border border-border rounded-lg text-sm text-text-body hover:bg-surface-hover transition"
        >
          <svg width="18" height="18" viewBox="0 0 18 18" aria-hidden="true">
            <path
              d="M17.64 9.2c0-.637-.057-1.251-.164-1.84H9v3.481h4.844c-.209 1.125-.843 2.078-1.796 2.717v2.258h2.908c1.702-1.567 2.684-3.875 2.684-6.615z"
              fill="#4285F4"
            />
            <path
              d="M9 18c2.43 0 4.467-.806 5.956-2.184l-2.908-2.258c-.806.54-1.837.86-3.048.86-2.344 0-4.328-1.584-5.036-3.711H.957v2.332A8.997 8.997 0 0 0 9 18z"
              fill="#34A853"
            />
            <path
              d="M3.964 10.707A5.41 5.41 0 0 1 3.682 9c0-.593.102-1.17.282-1.707V4.961H.957A8.996 8.996 0 0 0 0 9c0 1.452.348 2.827.957 4.039l3.007-2.332z"
              fill="#FBBC05"
            />
            <path
              d="M9 3.58c1.321 0 2.508.454 3.44 1.345l2.582-2.58C13.463.891 11.426 0 9 0A8.997 8.997 0 0 0 .957 4.961L3.964 6.293C4.672 4.169 6.656 3.58 9 3.58z"
              fill="#EA4335"
            />
          </svg>
          GOOGLEアカウントで登録
        </Button>

        {/* サインインリンク */}
        <p className="mt-6 text-sm text-text-secondary text-center">
          すでにアカウントをお持ちの方は{" "}
          <Link href="/users/signin" className="text-primary hover:underline">
            こちら
          </Link>
        </p>
      </div>
    </main>
  );
}
